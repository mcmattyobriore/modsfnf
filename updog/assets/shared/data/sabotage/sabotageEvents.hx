...
function onCreate()
{
	...
	investigationText.antialiasing = ClientPrefs.data.antialiasing;
...
function onBeatHit()
{
	...
	if (!StringTools.contains(anim, 'sing') && game.curBeat % 2 == 0)
		saboDetective.dance();
}
...
function sabotageCutscene2ndHalf() // this a little dire to look at but its ok
{
	...
	FlxTween.tween(FlxG.camera, {zoom: 1.3}, 0.75, {
		...
		{
			FlxTween.tween(FlxG.camera, {zoom: 1.25}, 0.75, {
