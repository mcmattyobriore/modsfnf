...
function onCreate()
{
	...
	buildings = new BGSprite(null, cliff.x + (996 * cliff.scale.x / 2),
		cliff.y + (549 * cliff.scale.y / 2)).loadFromSheet('stage/polus/meltdown/cutscene/bg', 'bg_building');
	buildings.scale.set(cliff.scale.x, cliff.scale.y);
	buildings.updateHitbox();
	cutsceneAssets.add(buildings);
	buildings.zIndex = -2;

	brdige = new BGSprite(null, cliff.x + (1664 * cliff.scale.x / 2),
		cliff.y + (600 * cliff.scale.y / 2)).loadFromSheet('stage/polus/meltdown/cutscene/bg', 'bridge');
	...
	lava = new BGSprite(null, cliff.x + (-383 * cliff.scale.x / 2),
		cliff.y + (2574 * cliff.scale.y / 2)).loadFromSheet('stage/polus/meltdown/cutscene/bg', 'bottom_lava');
	lava.scale.set(cliff.scale.x, cliff.scale.y);
	lava.updateHitbox();
	cutsceneAssets.add(lava);
	lava.zIndex = 2;

	lavaCover = new BGSprite(null, cliff.x + (-386 * cliff.scale.x / 2),
		cliff.y + (3155 * cliff.scale.y / 2)).loadFromSheet('stage/polus/meltdown/cutscene/bg', 'upper_lava');
	lavaCover.scale.set(cliff.scale.x, cliff.scale.y);
	lavaCover.updateHitbox();
	cutsceneAssets.add(lavaCover);
	lavaCover.zIndex = 99;

	crew = new BGSprite(null, cliff.x + (1060 * cliff.scale.x / 2),
		cliff.y + (398 * cliff.scale.y / 2)).loadFromSheet('stage/polus/meltdown/cutscene/bg', 'bg_crewmates_fuck..my_butt_hurts');
	...
	lavaSplash = new BGSprite(null, cliff.x + (1700 * cliff.scale.x / 2),
		cliff.y + (2700 * cliff.scale.y / 2)).loadFromSheet('stage/polus/meltdown/cutscene/lava_splash', 'lava splash');
...
function push()
{
	...
	FlxTween.tween(impostor, {y: impostor.y - 50}, 0.2, {
		...
		onComplete: Void ->
		{
			...
			FlxTween.tween(impostor.scale, {x: sc * 1.7, y: sc * 1.7}, 1, {
				ease: FlxEase.sineOut,
				onComplete: Void ->
				{
	...
	FlxTween.tween(camFollow, {y: camFollow.y - 25}, 0.2, {
		onComplete: Void ->
		{
